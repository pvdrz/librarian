...
impl Indices {
    pub(crate) fn insert(&mut self, id: DocId, doc: &Doc) {
        ...
        self.authors.insert_many(id, doc.authors.iter().map(|text| text.to_lowercase().into_bytes()));
        self.keywords.insert_many(id, doc.keywords.iter().map(|text| text.to_lowercase().into_bytes()));
    }
    ...
    pub fn search(&self, text: &str, limit: usize) -> Vec<(DocId, f32)> {
        ...
        for (id, score) in self.authors.search(text) {
            *scores.entry(id).or_insert(0.0) += score * 0.7;
        }

        for (id, score) in self.keywords.search(text) {
            *scores.entry(id).or_insert(0.0) += score * 0.5;
        }
